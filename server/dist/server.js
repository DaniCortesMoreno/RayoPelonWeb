...
app.post('/api/users', authMiddleware, requireRole(['ADMIN']), (req, res) => {
    ...
    const userRole = role === 'ADMIN' ? 'ADMIN' : 'MODERADOR';
    const db = Database.read();
    if (!db.users)
        db.users = [];
...
app.put('/api/users/:id', authMiddleware, requireRole(['ADMIN']), (req, res) => {
    ...
    if (username && String(username).trim().length >= 3) {
        const cleanUsername = String(username).trim();
        const conflict = db.users.find((u) => u.id !== id && u.username.toLowerCase() === cleanUsername.toLowerCase());
        if (conflict) {
...
// 3. Match Center
app.get('/api/matches/center', (req, res) => {
    const db = Database.read();
...
app.put('/api/matches/center', authMiddleware, (req, res) => {
    const db = Database.read();
...
app.post('/api/matches/reset-default', authMiddleware, (req, res) => {
    const db = Database.read();
    db.matches = [...INITIAL_MATCHES];
    db.matchCenter = computeMatchCenter(db.matches);
...
function sanitizePlayerData(body, existingPlayer) {
...
app.delete('/api/players/:id', authMiddleware, (req, res) => {
    const db = Database.read();
    const index = db.players.findIndex(p => p.id === req.params.id);
    if (index === -1)
        return res.status(404).json({ error: 'Jugador no encontrado' });
...
app.post('/api/upload/player-photo', authMiddleware, (req, res) => {
    try {
...
app.post('/api/news', authMiddleware, (req, res) => {
    try {
        ...
        if (!db.news)
            db.news = [];
        const { title, category, publishedAt, dateText, readTime, excerpt, content, author, authorRole, imageUrl, featured, medicalDetails } = req.body;
        if (!title || !category || !content) {
            return res.status(400).json({ error: 'Título, categoría y contenido son campos obligatorios' });
        }
        const validCategory = ['MEDICO', 'CRONICA', 'OFICIAL', 'NOVEDAD'].includes(category)
            ? category
            : 'OFICIAL';
...
app.put('/api/news/:id/feature', authMiddleware, (req, res) => {
    try {
        const db = Database.read();
        if (!db.news)
            db.news = [];
...
app.post('/api/upload/media-file', authMiddleware, (req, res) => {
    try {
...
app.get('/api/media/featured-match', (req, res) => {
    const db = Database.read();
    res.json(db.featuredMatch);
});
app.put('/api/media/featured-match', authMiddleware, (req, res) => {
    const db = Database.read();
    db.featuredMatch = {
        ...db.featuredMatch,
        ...req.body
...
app.post('/api/media/gallery', authMiddleware, (req, res) => {
    const db = Database.read();
    if (!db.gallery)
        db.gallery = [];
    const newPhoto = {
...
app.put('/api/media/gallery/:id', authMiddleware, (req, res) => {
    const db = Database.read();
    if (!db.gallery)
        db.gallery = [];
    const index = db.gallery.findIndex((p) => p.id === req.params.id);
    if (index === -1) {
...
app.delete('/api/media/gallery/:id', authMiddleware, (req, res) => {
    const db = Database.read();
    if (!db.gallery)
        db.gallery = [];
...
app.get('/api/media/clips', (req, res) => {
    const db = Database.read();
    res.json(db.clips || []);
});
app.post('/api/media/clips', authMiddleware, (req, res) => {
    const db = Database.read();
    if (!db.clips)
        db.clips = [];
...
app.put('/api/media/clips/:id', authMiddleware, (req, res) => {
    const db = Database.read();
    if (!db.clips)
        db.clips = [];
    const index = db.clips.findIndex((c) => c.id === req.params.id);
    if (index === -1) {
...
app.delete('/api/media/clips/:id', authMiddleware, (req, res) => {
    const db = Database.read();
    if (!db.clips)
        db.clips = [];
