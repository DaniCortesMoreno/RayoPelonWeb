...
app.get('/api/admin/backup', authMiddleware, requireRole(['ADMIN']), (req: AuthenticatedRequest, res: Response) => {
  const db = Database.read();
...
app.post('/api/admin/restore', authMiddleware, requireRole(['ADMIN']), (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (Array.isArray(backupData.players)) db.players = backupData.players;
    if (Array.isArray(backupData.news)) db.news = backupData.news;
    if (Array.isArray(backupData.matches)) {
      db.matches = backupData.matches;
      db.matchCenter = computeMatchCenter(db.matches || []);
    }
    if (backupData.featuredMatch) db.featuredMatch = backupData.featuredMatch;
    if (Array.isArray(backupData.gallery)) db.gallery = backupData.gallery;
    if (Array.isArray(backupData.clips)) db.clips = backupData.clips;
    if (Array.isArray(backupData.standings)) db.standings = backupData.standings;
